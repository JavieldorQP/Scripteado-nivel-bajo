...
void act_odom(Caracterizacion *situacion, T_Counter *contadores, param_mecanicos *mecanica) 
{
	...
	avance=(LPC_TIM2->TC*p_a_mm	 +	LPC_TIM3->TC*p_a_mm)	/	2;          				//c�lculo del avance en recto del robot
	deltaO=((LPC_TIM2->TC*p_a_mm  -	 LPC_TIM3->TC*p_a_mm)	 /  mecanica->L*10 ) * 180 / PI;		//c�lculo del incremento del �ngulo en grados
	
	deltaX						=		avance*cos(deltaO*PI/180);   			//calculo del avance en X del robot  (funcion cos en radianes)
	deltaY						=		avance*sin(deltaO*PI/180);				//calculo del avance en Y del robot  (funcion sin en radianes)
...
void EINT2_IRQHandler (void){			//C�digo de la interrupci�n del encoder de la rueda derecha 
	...
	if ((LPC_GPIO0->FIOPIN >> 23) & 0x1)					//Valor del pin 0.11
...
void EINT3_IRQHandler (void){			//C�digo de la interrupci�n del encoder de la rueda izquierda 
	...
	if ((LPC_GPIO0->FIOPIN >> 4) & 0x1)					//Valor del pin 0.11
