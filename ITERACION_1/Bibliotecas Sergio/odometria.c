...
posicion pose;								//Variable global que contiene los valores de posicion que se pasarán a alto nivel

void init_odom(void){					//Esta función inicializa las interrupciones externas y las GPIO que llevan los encoders en cuadratura.
	...
	LPC_SC->EXTPOLAR		|=	(1<<2)	| (1<<3) ;  //Defino el flanco de activavión como el flanco de subida
...
***********************************************************************************************************************************/
void EINT2_IRQHandler (void){			//Código de la interrupción del encoder de la rueda derecha 
...
void EINT3_IRQHandler (void){			//Código de la interrupción del encoder de la rueda izquierda 
...
{
	double deltaO, avance, deltaX, deltaY;		//definición de avance en recto del robot, incremento del ángulo, incremento en X e incremento en Y 

	
	avance=(pose.contador_derecho*p_a_cm	+	pose.contador_izquierdo*p_a_cm)	/	2;          						//cálculo del avance en recto del robot
	deltaO=((pose.contador_derecho*p_a_cm	-	pose.contador_izquierdo*p_a_cm)	/	LONG_EJE)*180/PI;		//cálculo del incremento del ángulo en grados
	...
	while (pos_actual.theta>=360)							// si se pasa de 360º convierte a rango 0
		pos_actual.theta-=360;								// no se contempla que de mas de una vuelta en un tiempo de muestreo
	while (pos_actual.theta<0)									// si se pasa de 360º convierte a rango 0
